...
async function initAuth() {
  ...
  if (!auth || !auth.loggedIn) { document.getElementById("navEmail").textContent = "Not logged in"; document.getElementById("navSignout").style.display = "none"; return; }
...
function renderBreakdown(id, data) {
  ...
  el.innerHTML = entries.map(([name, count]) => `<div class="cat-row"><div class="cat-name">${name.replace(/_/g, " ")}</div><div class="cat-bar-bg"><div class="cat-bar-fill" style="width:${(count / max) * 100}%"></div></div><div class="cat-count">${count}</div></div>`).join("");
}

async function loadProfile() {
  const res = await msg("GET_PROFILE"); const p = res?.profile || {};
...
document.getElementById("saveProfile").addEventListener("click", async () => {
  ...
  const profile = { ...existing, displayName: document.getElementById("profName").value.trim(), classes: classesRaw.split(",").map((c) => c.trim()).filter(Boolean), difficulty: document.getElementById("profDifficulty").value, sensitivity: parseInt(document.getElementById("profSensitivity").value) };
  const res = await msg("SAVE_PROFILE", { profile });
  toast(res?.error ? "Error: " + res.error : "Saved");
});

function renderKnowledge(dk) {
  const grid = document.getElementById("knowledgeGrid"); const entries = Object.entries(dk);
  if (!entries.length) return;
  grid.innerHTML = entries.sort((a, b) => (b[1].promptCount || 0) - (a[1].promptCount || 0))
    .map(([subj, d]) => `<div class="knowledge-chip"><h4>${subj}</h4><div class="kc">${d.promptCount || 0} prompts</div>${d.topics?.length ? `<div class="kt">${d.topics.slice(-6).join(" / ")}</div>` : ""}</div>`).join("");
}

(async () => { await initAuth(); await Promise.all([loadOverview(), loadProfile()]); })();
