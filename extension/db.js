...
export async function saveUserProfile(uid, idToken, profile) {
  return await setDoc(`users/${uid}`, profile, idToken);
}

export async function logPrompt(uid, idToken, promptData) {
  return await addDoc(`users/${uid}/prompts`, {
    ...promptData,
    timestamp: new Date().toISOString(),
  }, idToken);
}

export async function getDailyStats(uid, idToken, dateStr) {
  return await getDoc(`users/${uid}/dailyStats/${dateStr}`, idToken);
}

export async function saveDailyStats(uid, idToken, dateStr, stats) {
  return await setDoc(`users/${uid}/dailyStats/${dateStr}`, stats, idToken);
}
