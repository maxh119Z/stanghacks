...
async function handleSaveOnboarding(profile) {
  ...
  const fullProfile = {
    ...
    photoUrl: stored.photoUrl || "", classes: profile.classes || [],
    difficulty: profile.difficulty || "high_school", sensitivity: profile.sensitivity || 2,
    currentStatus: "", dynamicKnowledge: {},
...
async function handleSaveStatus(status) {
  ...
  await saveUserProfile(auth.uid, auth.idToken, { ...profile, lastActive: new Date().toISOString() });
  // Also cache locally for fast access during classification
...
async function handleGetPrompts(limit = 50) {
  const auth = await getValidToken();
  if (!auth) return { error: "Not logged in" };
  const prompts = await listPrompts(auth.uid, auth.idToken, limit);
  return { prompts };
}
...
async function handleClassification(promptText, images = [], site = "") {
  // Always read settings fresh (fixes API key "randomly stopping" bug)
  ...
  if (!profile) {
    profile = { classes: [], difficulty: "high_school", sensitivity: 2, currentStatus: "", dynamicKnowledge: {} };
  }

  // Also grab currentStatus from sync cache (faster than profile fetch)
  const cached = await chrome.storage.sync.get(["currentStatus"]);
  if (cached.currentStatus && !profile.currentStatus) {
    profile.currentStatus = cached.currentStatus;
  }
  ...
  // Async Firebase sync + badge update (don't block response)
  ...
  if (profile.dynamicKnowledge && Object.keys(profile.dynamicKnowledge).length > 0) {
    dynamicCtx = "\nDYNAMIC KNOWLEDGE (learned from usage):\n" +
      Object.entries(profile.dynamicKnowledge).map(([subj, d]) =>
        `  - ${subj}: ${d.promptCount} prompts${d.topics?.length ? ` (topics: ${d.topics.slice(-5).join(", ")})` : ""}`
      ).join("\n") + "\n";
  }
...
This means the student is currently focused on this activity. Be slightly more lenient with questions directly related to this status (e.g. concept clarification for their current study topic). However, DO NOT be lenient with direct answer seeking or homework completion — those are still high outsourcing risk regardless of status. The status only makes concept help and brainstorming more acceptable for that topic.\n`;
  }
...
function parseClassification(content) {
  try {
    ...
    return {
      intent_category: p.intent_category || "casual_chat", outsourcing_risk: p.outsourcing_risk || "low",
      confidence: Math.min(1, Math.max(0, parseFloat(p.confidence) || 0.5)), subject: p.subject || "general",
      recommended_intervention: p.recommended_intervention || "allow", message: p.message || "Processing...",
      hint: p.hint || null, profile_update: p.profile_update || null,
    };
  } catch (e) {
