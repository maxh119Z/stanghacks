(function () {
  ...
  async function extractImages() {
    ...
    for (const src of srcs) {
      try {
        ...
        c.width = Math.min(loaded.width, 800); c.height = Math.min(loaded.height, 800);
  ...
  function showLoading() {
    ...
    card.innerHTML = '<div class="bg-body" style="text-align:center;padding:32px 20px;"><p class="bg-message" style="margin:0;">Analyzing prompt...</p><div class="bg-meta" style="justify-content:center;margin-top:8px;"><span>This takes a moment</span></div></div>';
  ...
  function createOverlay(classification, promptText) {
    ...
    const overlay = document.createElement("div"); overlay.id = "think-overlay";
    const color = { low: "#4ec9b0", medium: "#cca700", high: "#f14c4c" }[classification.outsourcing_risk] || "#569cd6";
    ...
    document.getElementById("bg-send-anyway").addEventListener("click", () => { const b = document.getElementById("bg-send-anyway"); if (b && b.disabled) return; chrome.runtime.sendMessage({ type: "LOG_ACTION", action: "sent_anyway" }); removeOverlay(); bypassNext = true; clickSend(); });
  ...
  function clickSend() { setTimeout(() => { const btn = document.querySelector(config.sendButtonSelector); if (btn) btn.click(); }, 50); }
  ...
  async function handlePromptSubmit(e) {
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => { chrome.runtime.sendMessage({ type: "CLASSIFY_PROMPT", prompt: text, images, site: window.location.hostname }, (r) => { if (chrome.runtime.lastError) reject(chrome.runtime.lastError); else resolve(r); }); });
      if (result.error) { console.warn("[Think]", result.error); removeOverlay(); bypassNext = true; clickSend(); return; }
      if (result.recommended_intervention === "allow") { removeOverlay(); bypassNext = true; clickSend(); }
      else { createOverlay(result, text); }
    } catch (err) { console.error("[Think]", err); removeOverlay(); bypassNext = true; clickSend(); }
