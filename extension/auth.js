...
async function exchangeGoogleTokenForFirebase(googleIdToken) {
  const res = await fetch(`${AUTH_URL}/accounts:signInWithIdp?key=${FIREBASE_CONFIG.apiKey}`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ postBody: `id_token=${googleIdToken}&providerId=google.com`, requestUri: chrome.identity.getRedirectURL(), returnIdpCredential: true, returnSecureToken: true }),
  });
...
export async function getValidToken() {
  const stored = await chrome.storage.local.get(["idToken", "refreshTokenStr", "tokenExpiry", "uid"]);
  ...
  if (stored.tokenExpiry && Date.now() > stored.tokenExpiry - 300000) {
    try {
      ...
      await chrome.storage.local.set({ idToken: refreshed.idToken, refreshTokenStr: refreshed.refreshToken, tokenExpiry: Date.now() + 3600000, uid: refreshed.localId });
      return { idToken: refreshed.idToken, uid: refreshed.localId };
    } catch (e) { console.error("[Think] Token refresh failed:", e); return null; }
  }
  return { idToken: stored.idToken, uid: stored.uid };
}

export async function saveAuthToStorage(authData) {
  await chrome.storage.local.set({ idToken: authData.idToken, refreshTokenStr: authData.refreshToken, tokenExpiry: Date.now() + 3600000, uid: authData.localId, email: authData.email, displayName: authData.displayName || "", photoUrl: authData.photoUrl || "" });
}

export async function signOut() {
  await chrome.storage.local.remove(["idToken", "refreshTokenStr", "tokenExpiry", "uid", "email", "displayName", "photoUrl"]);
}
